...
void Lijst<T>::schrijf(ostream & os) const {
    ...
    if (this->get() != 0) {
        os << this->get()->sl;
        std::for_each(++begin(), end(), [&](const T & sl) {
            os << " . " << sl;
        });
    }
#else
...
//template<class T>
//Lijst<T>::Lijst(const Lijst<T>& ander) {
//    std::cout << "Called copy constructor" << endl;
//    this->reset();
//
//    if (ander) {
//        Lijstknoop<T> *l = new Lijstknoop<T>(ander->sl);
//        *this = std::unique_ptr<Lijstknoop < T >> (l);
//    }
//
//    // kopieer pointers recursief
//    this->get()->volgend = ander->volgend;
//
//    //*this = ander;
//}
...
template<class T>
Lijst<T>& Lijst<T>::operator=(const Lijst<T>& ander) {
    //std::cout << "Called copy assignment" << endl;
    ...
    if (ander) {
        //        Lijstknoopptr<T>::reset();
        //        Lijst<T>::voegToe(ander->sl);
        //        Lijst<T>* volgendeKnoop = &ander->volgend;
        //        while (*volgendeKnoop) {
        //            Lijst<T>::voegToe((*volgendeKnoop)->sl);
        //            volgendeKnoop = &(*volgendeKnoop)->volgend;
        //        }
//        Lijstknoop<T> *l = new Lijstknoop<T>(ander->sl);
//        *this = std::unique_ptr<Lijstknoop < T >> (l);
        Lijstknoopptr<T> l(new Lijstknoop<T>(ander->sl));
        *this = std::move(l);
    }
...
// move constructor
// called by std::swap(a, b)

template<class T>
Lijst<T>::Lijst(Lijst<T>&& ander) {
    //std::cout << "Called move constructor" << endl;
...
template<class T>
Lijst<T>& Lijst<T>::operator=(Lijst<T> &&ander) {
    //std::cout << "Called move assignment" << endl;
